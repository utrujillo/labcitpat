 ...
 */
 ...
 (function($){
	...
	$.fn.tablePagination = function(settings) {
		var defaults = {  
			firstArrow : (new Image()).src="img/first.png",  
			prevArrow : (new Image()).src="img/back.png",
			nextArrow : (new Image()).src="img/next.png",
			lastArrow : (new Image()).src="img/last.png",
			rowsPerPage : 5,
			currPage : 1,
			optionsForRows : [5,10,25,50,100],
			ignoreRows : [],
			classList: 'listPaginate'
		};  
      ...
      function createPaginationElements() {
        ...
        htmlBuffer.push("<select id='tablePagination_rowsPerPage' class='"+ defaults.classList +"'><option value='5'>5</option></select>");
        htmlBuffer.push("&nbsp;por Pagina");
        htmlBuffer.push("</span>");
        htmlBuffer.push("<span id='tablePagination_paginater'>");
        htmlBuffer.push("<img id='tablePagination_firstPage' src='"+defaults.firstArrow+"' style='cursor:pointer;'>");
        htmlBuffer.push("<img id='tablePagination_prevPage' src='"+defaults.prevArrow+"' style='cursor:pointer;'>");
        htmlBuffer.push("Pagina&nbsp;");
        htmlBuffer.push("<input id='tablePagination_currPage' type='input' value='"+currPageNumber+"' size='1' class='"+ defaults.classList +"'>");
        htmlBuffer.push("&nbsp;de&nbsp;<span id='tablePagination_totalPages'>"+totalPages+"</span>");
        htmlBuffer.push("<img id='tablePagination_nextPage' src='"+defaults.nextArrow+"' style='cursor:pointer;'>");
        htmlBuffer.push("<img id='tablePagination_lastPage' src='"+defaults.lastArrow+"' style='cursor:pointer;'>");
        htmlBuffer.push("</span>");
        htmlBuffer.push("</div>");
